...
export default function RegisterForm() {
  ...
  const handlePhoneNumberChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFieldError("phoneNumber", e.target.value);
  };
  ...
  const validationConditions: { [key: string]: (value: string) => boolean } = {
    ...
    phoneNumber: (value: string) => value.length >= 9,
    password: (value: string) => value.length >= 8,
    confirmPassword: (value: string) => value === password,
  };
  ...
  const handleSubmit = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    setIsLoading(true);
    await axios
      .post(`${apiConfig.user.baseUrl}/api/v1/auth/register`, {
        name,
        email,
        phoneNumber,
        username,
        ...
        <div className="row">
          <div className="col-lg-6">
            <div className="form-group first">
              ...
              />
              {errors.Name ? (
                <span className="text-xs text-danger ml-2">{errors.Name}</span>
              ) : null}
            </div>
            <div className="form-group first">
              <FormGroup
            ...
            <div className="form-group first">
              ...
              />
              {errors.Email ? (
                <span className="text-xs text-danger ml-2">{errors.Email}</span>
              ) : null}
            </div>
          </div>
          <div className="col-lg-6">
